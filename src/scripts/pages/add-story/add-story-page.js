...
export default class AddStoryPage {
  ...
  #toggleCameraElements(show) {
    ...
    document.querySelector(".video-controls").style.display = show
      ? "flex"
      : "none";
  }

  #removePicture(id) {
    const selectedPicture = this.#takenPicture.find(
      (picture) => picture.id === id
    );
    if (!selectedPicture) return null;

    this.#takenPicture = this.#takenPicture.filter(
      (picture) => picture.id !== id
    );
    ...
    inputPhoto.addEventListener("change", async (event) => {
      ...
      const imageBlob =
        file instanceof Blob
          ? file
          : await convertBase64ToBlob(file, "image/png");
  ...
  #setupMap() {
    ...
    const mapTilerLayer = L.tileLayer(
      "https://api.maptiler.com/maps/basic-v2/{z}/{x}/{y}.png?key=O1ZJB6XuHzgTSPNwgD5a",
      {
        attribution:
    ...
    mapText.innerHTML =
      "Silahkan klik tombol di atas ini untuk mendapatkan lokasi Anda.";
    ...
    getLocationButton.addEventListener("click", async (e) => {
      e.preventDefault();
      mapText.innerHTML =
        '<span class="spinner"></span> Sedang mengambil lokasi...';
      ...
      navigator.geolocation.getCurrentPosition(
        ...
        (error) => {
          console.error(error);
          mapText.innerText =
            "Tolong aktifkan permission lokasi terlebih dahulu.";
        },
        {
